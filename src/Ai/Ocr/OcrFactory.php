...
class OcrFactory
{
    ...
    public static function factory(object $config): ?object
    {
        try {
            ...
            if (!$class->isSubclassOf(__NAMESPACE__ . '\\OcrInterface')) {
                throw new ReflectionException($config->getSource() . "未实现OCR接口类");
            }
            $ocr = $class->newInstance();
            if ($ocr->checkConfig($config) !== true) {
                throw new Exception('未通过配置检查，请检查配置');
            }
            return $ocr;
        } catch (ReflectionException | Exception $e) {
            log_message('error', 'OCR源{ocr}加载失败，error：{error}',
                [
                    'ocr' => $config->getSource(),
                    'error' => $e->getMessage()
                ]
            );
            return null;
